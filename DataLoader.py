...
def is_filename(input_path: str = None):
    ...
    # 处理路径结尾的分隔符（如 'path/to/dir/' 判定为目录）
    if input_path.endswith(os.sep) or input_path.endswith('/'):
        return False
    # 提取文件名部分并检查是否有扩展名
    ...
    if os.path.isdir(input_path):
        ...
        if not obj_path_list:
            ...
            if image_files:
                ...
                output_folder = os.path.join(output_path, current_time)
                os.makedirs(output_folder, exist_ok=True)
                temp_folder_path = os.path.join("temp", current_time)
                os.makedirs(temp_folder, exist_ok=True)

                for image_path in image_files:
                    img_format = os.path.splitext(image_path)[1].lower()
                    img_name = f"{datetime.now().strftime('%H%M%S')}{img_format}"
                    output_img_path = os.path.join(output_folder, img_name)
                    temp_img_path = os.path.join(temp_folder_path, img_name)
                    ...
                    # 创建一个空的BuildingObj对象
                    empty_building_object = BuildingObj(None, None, temp_folder, output_folder)
                    texture_image = timg(image_path)
        ...
        if input_path.endswith('.obj'):
            ...
            shutil.copy2(mtl, temp)
            yield from load_building([(obj, mtl, temp, output)])
            return
